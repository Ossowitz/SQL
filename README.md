...
*Возвращает все строки таблицы в левой части соединения и соответствующие строки для таблицы в правой части соединения. **Строки, для которых нет соответствующей строки с правой стороны, результирующий набор будет содержать null.***
...
FULL OUTER JOIN соединяет одну строку первой таблицы с определённой строкой второй таблицы, а при CROSS JOIN каждая строка одной таблицы соединяется с каждой строкой другой таблицы.
