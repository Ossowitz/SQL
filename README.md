...
## Различия паттернов DAO и Репозиторий

• Похожие понятия (и то, и другое для работы с данными) <br/>
• Репозиторий обычно более высокоуровневый, ближе к бизнес-логике (не пишем SQL-запросы, работаем с сущностями) <br/>
• DAO обычно более низкоуровневый, ближе к БД (можем писать SQL-запросы) <br/>
• В сложных приложениях есть и то, и другое. **Репозиторий - для стандартных операций с данными (CRUD, например), DAO - 
для более сложных манипуляций с данными и с БД, где обычно нужно вручную писать SQL** <br/>
...
• В контроллере мы хотим иметь минимум кода. В идеале - только простейшая логика обработки запросов и ответов. <br/>
• Любая работа с данными, полученными из БД должна лежать в **сервисном слое**. Здесь мы описываем логику нашего приложения, а в контроллере только вызываем нужные методы. <br/>
• В один сервисный класс может быть внедрено несколько DAO, репозиториев<br/>
• Класс для сервисного слоя помечается аннотацией @Service <br/>
